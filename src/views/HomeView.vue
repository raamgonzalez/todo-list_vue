...
<template>
  <main class="font-serif">
    <div class="container__child flex flex-row gap-5 place-items-center mt-4">
        ...
        <button autofocus class="button__add w-full h-[34px] shadow-[#00808084] rounded-md text-l py-1 px-4 shadow-sm hover:scale-105 transition ease-in-out text-slate-700" type="add" @click="addAndNavigate(name)">Add</button>
    </div>
    <div class="container__child my-6 flex flex-col gap-6 mt-10">
      <h2 class="text-2xl border-b-2 border-slate-300">Task</h2>
      ...
      <ul id="incomplete-tasks" class="flex flex-col gap-6">
        <li v-for="todo in todosIncompleted" :key="todo.id" @click="toggle(todo.id)" class="flex flex-row justify-between gap-8 place-items-center w-full">
          ...
          <label class=" text-left">{{ todo.name }}</label>
          <button class="delete rounded-md py-1 px-2 shadow-[#00808084] shadow-sm hover:scale-105 transition ease-in-out text-slate-700 text-l" @click= "deleteTodo(todo.id)">Delete</button>
        </li>
      </ul>
      <h2 class=" text-2xl border-b-2 border-slate-300">Completed</h2>
      <ul id="completed-tasks" class="flex flex-col gap-6">
        <li v-for="todo in todosCompleted" :key="todo.id" @click="toggle(todo.id)" class="flex flex-row justify-between gap-8 place-items-center w-full">
          ...
          <label class=" text-left line-through">{{ todo.name }}</label>
          <button class="delete delete rounded-md py-1 px-2 shadow-[#00808084] shadow-sm hover:scale-105 transition ease-in-out text-slate-700 text-l" @click= "deleteTodo(todo.id)">Delete</button>
        </li>
      </ul>
    </div>
  </main>
